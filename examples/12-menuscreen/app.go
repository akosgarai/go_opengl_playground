...
func (c *Charset) Print(text string, x, y, scale float32) {
	...
	for i := range indices {
		...
		w := float32(ch.bearingWidth) * scale
		h := float32(ch.bearingHeight) * scale
		rect := rectangle.New(float32(ch.glyphWidth), float32(ch.glyphHeight))
		cols := []mgl32.Vec3{
			mgl32.Vec3{0.0, 1.0, 0.0},
		}
		v, i, _ := rect.TexturedColoredMeshInput(cols)
		position := mgl32.Vec3{xpos + w/2, 0, ypos + h/2}
		msh := mesh.NewTexturedColoredMesh(v, i, ch.tex, cols, glWrapper)
		msh.SetScale(mgl32.Vec3{scale, 1, scale})
		msh.SetPosition(position.Mul(scale))
		c.Model.AddMesh(msh)
		fmt.Printf("pos: %#v\nch: %#v\nw: %f, h: %f, xpos: %f, ypos: %f, adv: %f\n\n", position.Mul(scale), ch, w, h, xpos, ypos, float32((ch.advance>>6))*scale)
...
func main() {
	...
	textureShader := shader.NewShader(baseDir()+"/shaders/font.vert", baseDir()+"/shaders/font.frag", glWrapper)
	...
	Fonts.Print("How are You?", 0, 0, 0.05)
	Fonts.RotateX(-90)
	Fonts.SetTransparent(true)
	app.AddModelToShader(Fonts, textureShader)
