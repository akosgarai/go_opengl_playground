...
func InitSettings() {
	...
	Settings.AddConfig("CubeRotation", "Cube rotation", "The rotation speed of the cube mesh.", float32(0.2), nil)
	// camera options:
	// - position
...
// It generates a cube.
func CreateCubeMesh(t texture.Textures) *mesh.TexturedColoredMesh {
	colors := []mgl32.Vec3{
	...
	Cube = mesh.NewTexturedColoredMesh(v, i, t, colors, glWrapper)
	Cube.SetPosition(Settings["CubePosition"].GetCurrentValue().(mgl32.Vec3))
	return Cube
}
...
func Update() {
	...
	lastUpdate = nowNano
	rotationAngle := float32(delta) * Settings["CubeRotation"].GetCurrentValue().(float32)
