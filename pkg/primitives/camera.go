...
func LookAt(eye, lookAt, worldUp Vector) *Matrix4x4 {
	...
	result.Points[8] = u.Z
	result.Points[9] = v.Z
	result.Points[10] = w.Z
	result.Points[15] = 1
	translationMatrix := TranslationMatrix4x4(-eye.X, -eye.Y, -eye.Z)
...
func (c *Camera) TargetCameraUpdate() {
	...
	c.lookAt = (c.target.Subtract(c.position)).Normalize()
...
func (c *Camera) TargetCameraSetTarget(target Vector) {
	...
	if c.viewMatrix.Points[0] < 0 {
		...
		c.targetCameraOptions.yaw = RadToDeg(math.Pi - math.Asin(-c.viewMatrix[8]))
	} else {
		// m_yaw = glm::degrees(asinf(-V[2][0]));
		c.targetCameraOptions.yaw = RadToDeg(math.Asin(-c.viewMatrix[8]))
	}
	// m_pitch = glm::degrees(asinf(-V[1][2]));
	c.targetCameraOptions.pitch = RadToDeg(math.Asin(-c.viewMatrix[6]
}
